...
// 6. Return new array with format: "Drug: [name] - [dosageMg]mg"
const drugDescriptions = drugs.map(drug => `Drug: ${drug.name} - ${drug.dosageMg}mg`);
console.log("Formatted Drug Info:", drugDescriptions);
...
// 9. Function that takes a manufacturer name and returns how many drugs are from that company.
function countDrugsByManufacturer(manufacturerName) {
  return drugs.filter(drug => drug.manufacturer === manufacturerName).length;
}
console.log("Drugs from Pfizer:", countDrugsByManufacturer("Pfizer"));
